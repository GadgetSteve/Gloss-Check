...
def get_file_lists(start_dir="."):
    ...
    for root, dirs, files in os.walk(start_dir):
        ...
        glossary_list.update(
            [os.path.join(root, fn) for fn in fnmatch.filter(files, "glossary*.rst")]
        )
        glossary_list.update(
            [os.path.join(root, fn) for fn in fnmatch.filter(files, "glossary*.txt")]
        )
        file_list = file_list
...
def main(startdir=".", lang="en_UK"):
    ...
    for path in file_list:
        ...
        bad = [
            ...
            if not checker.check(word) and word not in glossaries
        ]
        if len(bad) == 0:
            print("OK")
        else:
            ...
            print(
                "\n".join(
                    textwrap.wrap(", ".join(sorted(bad, key=lambda s: s.lower())), 78)
                )
            )
